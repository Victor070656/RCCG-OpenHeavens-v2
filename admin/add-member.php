...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($first_name)) {
        ...
        if (empty($error)) {
            $stmt = $conn->prepare("INSERT INTO members (first_name, last_name, email, phone, address, date_of_birth, gender, membership_date, membership_type, department, photo, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
            $stmt->bind_param("ssssssssssss", $first_name, $last_name, $email, $phone, $address, $date_of_birth, $gender, $membership_date, $membership_type, $department, $photo, $status);
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <select class="form-select" id="membership_type" name="membership_type">
                                    ...
                                    <option value="associate_member" <?php echo (isset($_POST['membership_type']) && $_POST['membership_type'] == 'associate_member') ? 'selected' : ''; ?>>Associate Member</option>
